...
// etag6: runs the query for the generated file pub_module_level_const_missing.json
pub(super) fn run_check_release(
  ...
  let version_change = release_type.map(Into::into)
    .or_else(|| classify_semver_version_change(current_version, baseline_version))
    .unwrap_or_else(|| {config.shell_warn("Could not determine whether crate version changed. Assuming no change.",).expect("print failed");ActualSemverUpdate::NotChanged});
  ...
  let queries = SemverQuery::all_queries(); // 1. gets macro-generated query-functions that read query .ron files
  ...
  let queries_to_run: Vec<_> = queries.iter()
  ...
  config.shell_status("Checking",format_args!("{crate_name} v{} -> v{} ({}{} change)",
                         baseline_version.unwrap_or("unknown"),
                         current_version .unwrap_or("unknown"        ),
                                                                    assume,change),)?;
  config.verbose(|config| {config.shell_status("Starting",format_args!("{} checks, {} unnecessary",
    queries_to_run.len(),skipped_queries),)}).expect("print failed");
  ...
  for (query_id, semver_query) in queries_to_run.iter().copied() {
    ...
    config.verbose(|config| {if config.is_stderr_tty() {
      colored!(config.stderr(),"{}{}{:>12}{} [{:9}] {:^18} {}",fg!(Some(Color::Cyan)),
        bold!(true),"Running",reset!(),"",category,query_id,)?;config.stderr().flush()?;}
        Ok(())
      }).expect("print failed");
    ...
    if peeked.is_none() {
      config.verbose(|config| {if config.is_stderr_tty() {write!(config.stderr(), "\r")?;}
          colored_ln(config.stderr(), |w| {
            colored!(w,"{}{}{:>12}{} [{:>8.3}s] {:^18} {}",
              fg!(Some(Color::Green)),bold!(true),"PASS",reset!(),time_to_decide.as_secs_f32(),category,query_id,)})?;
          Ok(())}).expect("print failed");
    } else { queries_with_errors.push(QueryWithResults::new(query_id.as_str(), results_iter));
      config.verbose(|config| {if config.is_stderr_tty() {write!(config.stderr(), "\r")?;}
          colored_ln(config.stderr(), |w| {colored!(w,"{}{}{:>12}{} [{:>8.3}s] {:^18} {}",
              fg!(Some(Color::Red)),bold!(true),"FAIL",reset!(),time_to_decide.as_secs_f32(),category,query_id,
            )})?;Ok(())}).expect("print failed");
    }
  }

  if !queries_with_errors.is_empty() {config.shell_print("ErrCompleted",
        format_args!(
          ...
          skipped_queries,
        ),Color::Red,true,).expect("print failed");
    ...
    let required_bump = if required_versions.contains(&RequiredSemverUpdate::Major) {RequiredSemverUpdate::Major
    } else if required_versions.contains(&RequiredSemverUpdate::Minor) { RequiredSemverUpdate::Minor
    } else { unreachable!("{:?}", required_versions)};

    config.shell_print("Final",format_args!(
  ...
  } else {config.shell_print("Completed",format_args!(
    "[{:>8.3}s] {} checks; {} passed, {} unnecessary",
    total_duration.as_secs_f32(),queries_to_run.len(),queries_to_run.len(),skipped_queries,)
    , Color::Green,true,).expect("print failed");
    Ok(CrateReport {
      detected_bump: version_change,
      required_bump: None,
    })
