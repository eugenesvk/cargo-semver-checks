...
#[derive(Debug)] enum RustdocSource {
  Rustdoc(PathBuf)                   	,// Path to the Rustdoc json file Use this option when you have already generated the rustdoc file.
  Root(PathBuf)                      	,// Project root directory, i.e. the directory containing the crate source It can be a workspace or a single package.
  Revision(PathBuf, String)          	,// Project root directory and Git Revision.
  VersionFromRegistry(Option<String>)	,// Version from cargo registry to lookup. E.g. "1.0.0". If `None`, uses the largest-numbered non-yanked non-prerelease version published to the cargo registry. If no such version, uses the largest-numbered version including yanked and prerelease versions.
}
...
impl Check {
  ...
  // parse cargo.toml
  fn get_rustdoc_generator(&self, config: &mut GlobalConfig, source: &RustdocSource,) -> anyhow::Result<Box<dyn rustdoc_gen::RustdocGenerator>> {
    let target_dir = self.get_target_dir(source)?;
    // println!("{:?} {:?}",root,&target_dir);
    Ok(match source {
      // Root("..test_crates\\pub_module_level_const_missing\\new")
      RustdocSource::Root    (root     )	=> {println!("×b {:?}",source); Box::new(rustdoc_gen::RustdocFromProjectRoot::new(root, &target_dir)?)}
      RustdocSource::Rustdoc (     path)	=> {println!(" a");             Box::new(rustdoc_gen::RustdocFromFile       ::new(path.to_owned()))}
      RustdocSource::Revision(root, rev)	=> {println!(" c");
        let metadata = manifest_metadata_no_deps(root)?;
        let source  = metadata.workspace_root.as_std_path();
        Box::new(rustdoc_gen::RustdocFromGitRevision::with_rev(source,&target_dir,rev,config,)?)
      }
  ...
  // etag5: contains calls generate_versioned_crates to call @rustdoc_gen to generate rustdoc actual file pub_module_level_const_missing
  pub fn check_release(&self) -> anyhow::Result<Report> {
    let mut config = GlobalConfig::new().set_level(self.log_level);
    let rustdoc_cmd = RustdocCommand::new().deps(false).silence(!config.is_verbose()); //RustdocCommand{deps:false,silence:true}
    ...
    { //println!("×××× matches");
      ...
      match rustc_version::version() {
        Ok(rustc_version) => { if rustc_version < *rustc_version_needed {
        ...
        Err(error) => { let help = format!("HELP: to avoid errors please ensure rustc >={rustc_version_needed} is used");
          config.shell_warn(format_args!("failed to determine the current rustc version: {error}\n\n{help}"))?;}
      };}

    // get parsed parse cargo.toml
    let current_loader  = self.get_rustdoc_generator(&mut config, &self.current.source )?;
    // let tempprint = *&current_loader; println!("current_loader {:?}",tempprint.project_root);
    let baseline_loader = self.get_rustdoc_generator(&mut config, &self.baseline.source)?;

    // Create a report for each crate. We want to run all the checks, even if one returns `Err`.
    let all_outcomes: Vec<anyhow::Result<(String, Option<CrateReport>)>> = match &self
      .current
      .source
    {
      RustdocSource::Rustdoc(_)| RustdocSource::Revision(_, _)| RustdocSource::VersionFromRegistry(_) => {
        println!("×××× Rustdoc Revision VersionFromRegistry");
        let names = match &self.scope.mode {ScopeMode::DenyList(_) =>
            match &self.current.source { RustdocSource::Rustdoc(_) => // This is a user-facing string. For example, it appears when two pre-generated rustdoc files are semver-checked against each other.
        ...
        names.into_iter().map(|name| {
          let version = None;
          let (current_crate, baseline_crate) = generate_versioned_crates(&mut config,&rustdoc_cmd,&*current_loader,&*baseline_loader,
            CrateDataForRustdoc {crate_type: rustdoc_gen::CrateType::Current,name: &name,feature_config: &self.current_feature_config,},
            CrateDataForRustdoc {crate_type: rustdoc_gen::CrateType::Baseline {highest_allowed_version: version,},name: &name,feature_config: &self.baseline_feature_config,},)?;
            let report = run_check_release(&mut config,&name,current_crate,baseline_crate,self.release_type,)?;
            Ok((name, Some(report)))
          }).collect()
      }
      // etag4: calls generate_versioned_crates to call @rustdoc_gen to generate rustdoc actual file pub_module_level_const_missing
      RustdocSource::Root(project_root) => {
        ...
        if selected.is_empty() {anyhow::bail!("no crates with library targets selected, nothing to semver-check");}
        selected.iter().map(|selected| {
          let crate_name = &selected.name;
          let version = &selected.version;
          println!("×××× crate_name {:?} version={:?}",crate_name,version); // ×××× crate_name "cargo-semver-checks" version=Version { major: 0, minor: 23, patch: 0 }
          // println!("config {:?}",config); // doesn't implement debug
          let (current_crate, baseline_crate) = generate_versioned_crates( // println!("×××× not implied, deleted that");
            &mut config,&rustdoc_cmd,
            &*current_loader,&*baseline_loader,
            CrateDataForRustdoc {crate_type:rustdoc_gen::CrateType::Current,
              name:crate_name,feature_config: &self.current_feature_config,},
            CrateDataForRustdoc {crate_type:rustdoc_gen::CrateType::Baseline {highest_allowed_version:Some(version),},
              name:crate_name,feature_config: &self.baseline_feature_config,},
          )?;
          Ok((crate_name.clone(), // etag6 ↓ this runs the query based on versioned crate read from generated doc
              Some(run_check_release(&mut config,crate_name,
              current_crate,baseline_crate,self.release_type,)?),
            ))}).collect()
      }
    };
...
// etag3: calls ↓ @rustdoc_gen to generate rustdoc actual file pub_module_level_const_missing
  // impl RustdocGenerator for RustdocFromProjectRoot {fn load_rustdoc(&self,
fn generate_versioned_crates(
  ...
  let current_crate = load_rustdoc(&current_path)?; // external crate trustfall_rustdoc !!!!! this is the doc!// println!("@generate_versioned_crates current_crate{:?}",current_crate);
    // pub fn load_rustdoc(path: &Path) -> Result<VersionedCrate>

  // version check before returning
  let current_rustdoc_version = current_crate.version(); // 26
  let baseline_path = get_baseline_rustdoc_path(config,rustdoc_cmd,baseline_loader,baseline_crate_data.clone(),)?;
  // println!("@generate_versioned_crates\n  current_path{:?}\n  baseline_path{:?}",current_path.as_os_str(),baseline_path.as_os_str(),);
    // current_path".cache\\cargo\\semver-checks\\target\\doc/pub_module_level_const_missing.json"
    // baseline_path same↑
  let baseline_crate = {
    let mut baseline_crate = load_rustdoc(&baseline_path)?; // The baseline rustdoc JSON  may have been cached; ensure its rustdoc version matches the version emitted by the currently-installed toolchain. The baseline and current rustdoc JSONs should have the same version. If the baseline rustdoc version doesn't match, delete the cached baseline and rebuild it. Fix for: https://github.com/obi1kenobi/cargo-semver-checks/issues/415
    if baseline_crate.version() != current_rustdoc_version {
      let crate_name = baseline_crate_data.name;
      config.shell_status("Removing",format_args!("stale cached baseline rustdoc for {crate_name}"),)?;
      std::fs::remove_file(baseline_path)?;
      let baseline_path = get_baseline_rustdoc_path(config,rustdoc_cmd,
        baseline_loader,baseline_crate_data,)?;
      baseline_crate = load_rustdoc(&baseline_path)?;
      assert_eq!(baseline_crate.version(),current_rustdoc_version,"Deleting and regenerating the baseline JSON file did not resolve the rustdoc version mismatch.");
    }
